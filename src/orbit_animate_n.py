...
def make_random_bodies():
    ...
    for i in range(N):
        mass    = np.random.uniform(1e24, 2e30)   # zufällige Masse
        angle   = np.random.uniform(0, 2*np.pi)
        dist    = np.random.uniform(0.5*AU, 5*AU)
        pos     = np.array([np.cos(angle)*dist, np.sin(angle)*dist])

        # Kreisbahngeschwindigkeit: v = sqrt(G * M_total / r)
        # vereinfacht: wir nehmen Gesamtmasse ~ 2e30
        v_circ  = np.sqrt(G * 2e30 / dist) * np.random.uniform(0.7, 1.1)
        tang    = np.array([-np.sin(angle), np.cos(angle)])  # tangential
        vel     = tang * v_circ

        sizes.append(max(4, int(np.log10(mass) - 20)))
        bodies.append({
            "name":  f"K{i+1}",
            "mass":  mass,
            "pos":   pos,
...
def update(frame):
    ...
    for i, b in enumerate(bodies):
        x, y = b["pos"]
        trail_x[i].append(x); trail_y[i].append(y)
        if len(trail_x[i]) > TRAIL: trail_x[i].pop(0); trail_y[i].pop(0)
        dots[i].set_data([x], [y])
        trails[i].set_data(trail_x[i], trail_y[i])
        labels[i].set_position((x + 0.2*AU, y + 0.2*AU))
    years = day[0] * dt / (365.25 * 86400)
    title.set_text(f"N-Körper · RK4 · {N} Körper  —  Tag {day[0]}  ({years:.2f} Jahre)")
